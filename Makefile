...
docker-debian8: depends scratch
	sudo /usr/share/docker.io/contrib/mkimage.sh -t cbayle/debian:8 debootstrap --variant=minbase jessie http://cdn.debian.net/debian
...
run8:
	docker run --rm=true -t -i cbayle/debian:8 /bin/bash
